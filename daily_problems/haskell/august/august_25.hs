...
transpose :: [[a]] -> [[a]]
transpose cs = [ [ ((cs !! j) !! i) | j <- [0..m] ] | i <- [0..n] ]
    where
        m = (pred . length) cs
        n = (pred . length) (head cs)

removeNonOrdered :: (Ord a) => [[a]] -> Int
removeNonOrdered cs = (length transposed) - (length ordered)
    where
        transposed = transpose cs
        ordered = filter isOrdered transposed

isOrdered :: (Ord a) => [a] -> Bool
isOrdered (l:ls) = isOrdered' l ls
